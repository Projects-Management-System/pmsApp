...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/vendorProjectsDatabasesPendingTasks/Huawei`, {
      params: {
        Implementation_By: 'Huawei',
        Site_Engineer: DropDownValue,
        Project: ProjectNameDropdownValue
      }
    });
    setState(res.data.TeamAllocationPendingTasks);
  };
  // ---------------------------------------------------------
              ...
              <IconButton
                onClick={() => {
                  ...
                  if (selectedIDs.length === 1) {
                    navigate(
                      `/dashboard/DatabasesVendorProjects/Huawei/TeamAllocation/Edit/${selectionModel}`
                    );
  ...
  const deleteUser = async (selectionModel) => {
    await axiosInstance.delete(`/vendorProjectsDatabases/delete/${selectionModel}`).catch((err) => {
  ...
  const handleEditRowsModelChange = React.useCallback(
    (model) => {
      ...
      if (editedIds.length === 0) {
        ...
        if (confirmBox === true) {
          axiosInstance.put(`/DatabasesVendorProjects/Edit/${editRowData._id.value}`, newPost);
