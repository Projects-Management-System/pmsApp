...
export default function Datagrid({ DropDownValue, ProjectNameDropdownValue }) {
  ...
  const fetchData = async () => {
    const res = await axiosInstance.get(`/vendorProjectsDatabasesSiteData/ZTE`, {
      params: {
        Implementation_By: 'ZTE',
              ...
              <IconButton
                onClick={() => {
                  ...
                  if (selectedIDs.length === 1) {
                    navigate(
                      `/dashboard/DatabasesVendorProjects/ZTEProjects/Edit/${selectionModel}`
                    );
